...
async def on_ready():
    ...
    await bot.change_presence(status=discord.Status.online,
                              activity=discord.Game("Cyberpunk RED"))
...
async def about(ctx):
    ...
    :type ctx: discord.ext.commands.context.Context
    """
    embed = discord.Embed(title="Glitch",
                          description="A Discord bot for Cyberpunk RED",
                          color=0x2c84fa)
    embed.set_thumbnail(url="https://i.imgur.com/cRIB0hg.jpg")
    embed.add_field(name="Original Developer", value="John 'JT' Thomas",
                    inline=False)
    embed.add_field(name="Current Developer", value="Frank Pasqualini",
                    inline=False)
    embed.add_field(name="Documentation", value="https://glitch.red/",
                    inline=False)
    embed.add_field(name="GitHub",
                    value="https://github.com/frank-pasqualini/glitch/",
                    inline=False)
    embed.set_footer(
        text="The bot and its authors are not affiliated with nor endorsed "
             "by R. Talsorian Games.")
    await ctx.send(embed=embed)


@bot.command(aliases=["market", "nm"],
             help="Randomly generates a Night Market")
async def night_market(ctx):
    ...
    :type ctx: discord.ext.commands.context.Context
    """
    market, cats = generate_night_market()
    cat_1 = str(' '.join(word.title() for word in cats[0].__name__.split('_')))
    cat_2 = str(' '.join(word.title() for word in cats[1].__name__.split('_')))
    out = "This market is selling " + cat_1 + " and " + cat_2 + "\n"
    out += "```\n"
...
@bot.command(aliases=["r"],
             help="Rolls the amount of type of dice specified. Ex: 1d10, 5d6")
