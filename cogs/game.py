...
class Game(commands.Cog):
	...
	async def skill_check(self, ctx, skill_str, *args):
		...
		if skill_exists and skill_str in character:
			...
			DB.collection("users").document(str(ctx.message.author.id)).collection("characters").document(active).set({skill_str: 0}, merge=True)
			skill_level = 0
			stat_level = int(character[stat])
		else:
		...
		results += "\n" + stat.capitalize() + ": " + str(stat_level) + "|" + skill_str.capitalize() + ": " + str(skill_level) + "|Roll: " + str(roll) + "|LUCK: " + str(luck) + "|Mod: " + str(mod)
		if roll == 10:
