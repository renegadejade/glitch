...
def read_json(filename, relate, cost, subtype=None):
    ...
    with open("night_market/" + filename + ".json") as json_file:
        ...
        for i in item_list:
            if relate(int(i["cost"]), cost):
                if not subtype or subtype == i["type"]:
                    return_list += [(i["name"], i["cost"])]
...
def generate_night_market():
    """
    Rolls 2 separate numbers on a D6 and appends the items from that category
    to the market list.
    ...
    def food_and_drugs(fad_roll):
        ...
        sample = random.sample(list(fad_switcher.values()), fad_roll)
        return list(chain.from_iterable(sample))
    ...
    def personal_electronics(pe_roll):
        ...
        pe_switcher = {
            ...
            6: [("Computer", 50)],
            7: [("Poor Quality Cyberdeck", 100),
                ("Cyberdeck", 500),
                ("Excellent Quality Cyberdeck", 1000)],
        ...
        sample = random.sample(list(pe_switcher.values()), pe_roll)
        return list(chain.from_iterable(sample))
    ...
    def weapons_and_armor(waa_roll):
        ...
        waa_switcher = {
            1: [("Medium Pistol", 50),
                ("Poor Quality Medium Pistol", 20),
                ("Excellent Quality Pistol", 100)],
            2: [("Heavy Pistol", 100),
                ("Poor Quality Heavy Pistol", 50),
                ("Excellent Quality Heavy Pistol", 500),
                ("Very Heavy Pistol", 100),
                ("Poor Quality Very Heavy Pistol", 50),
                ("Excellent Quality Very Heavy Pistol", 500)],
            3: [("SMG", 100),
                ("Poor Quality SMG", 50),
                ("Excellent Quality SMG", 500)],
            4: [("Heavy SMG", 100),
                ("Poor Quality Heavy SMG", 50),
                ("Excellent Quality Heavy SMG", 500)],
            5: [("Shotgun", 500),
                ("Poor Quality Shotgun", 100),
                ("Excellent Quality Shotgun", 1000)],
            6: [("Assault Rifle", 500),
                ("Poor Quality Assault Rifle", 100),
                ("Excellent Quality Assault Rifle", 1000)],
            7: [("Sniper Rifle", 500),
                ("Poor Quality Sniper Rifle", 100),
                ("Excellent Quality Sniper Rifle", 1000)],
            8: [("Bow", 100),
                ("Poor Quality Bow", 50),
                ("Excellent Quality Bow", 500),
                ("Crossbow", 100),
                ("Poor Quality Crossbow", 50),
                ("Excellent Quality Crossbow", 500)],
            9: [("Grenade Launcher", 500),
                ("Poor Quality Grenade Launcher", 100),
                ("Excellent Quality Grenade Launcher", 1000),
                ("Rocket Launcher", 500),
                ("Poor Quality Rocket Launcher", 100),
                ("Excellent Quality Rocket Launcher", 1000)],
            10: read_json("ammunition", operator.le, 500),
            11: random.sample(read_json("exotic_weapons", operator.ge, 0), 1),
            12: [("Light Melee Weapon", 50),
                 ("Poor Quality Light Melee Weapon", 20),
                 ("High Quality Light Melee Weapon", 100)],
            13: [("Medium Melee Weapon", 50),
                 ("Poor Quality Medium Melee Weapon", 20),
                 ("High Quality Medium Melee Weapon", 100)],
            14: [("Heavy Melee Weapon", 100),
                 ("Poor Quality Heavy Melee Weapon", 50),
                 ("High Quality Heavy Melee Weapon", 500)],
            15: [("Very Heavy Melee Weapon", 100),
                 ("Poor Quality Very Heavy Melee Weapon", 50),
                 ("Excellent Quality Very Heavy Melee Weapon", 500)],
        ...
        sample = random.sample(list(waa_switcher.values()), waa_roll)
        return list(chain.from_iterable(sample))
    ...
    def cyberware(cw_roll):
        ...
        cw_switcher = {
            ...
            10: [("Cybereye", 100)] +
                read_json("cyberware", operator.eq, 1000, "Cyberoptics"),
            11: [("Cybereye", 100)] +
                read_json("cyberware", operator.le, 500, "Cyberoptics"),
            12: [("Cyberaudio Suite", 500)] +
                read_json("cyberware", operator.eq, 1000, "Cyberaudio"),
            13: [("Cyberaudio Suite", 500)] +
                read_json("cyberware", operator.le, 500, "Cyberaudio"),
            14: [("Neural Link", 500), ("Chipware Socket", 500)] +
                read_json("cyberware", operator.eq, 1000, "Neuralware"),
            15: [("Neural Link", 500), ("Chipware Socket", 500)] +
                read_json("cyberware", operator.eq, 1000, "Neuralware"),
            16: [("Cyberarm", 500), ("Cyberleg", 500)] +
                read_json("cyberware", operator.eq, 1000, "Cyberlimbs"),
            17: [("Cyberarm", 500), ("Cyberleg", 500)] +
                read_json("cyberware", operator.le, 500, "Cyberlimbs"),
        ...
        sample = random.sample(list(cw_switcher.values()), cw_roll)
        return list(chain.from_iterable(sample))
    ...
    def clothing_and_fashionware(caf_roll):
        ...
        sample = random.sample(list(caf_switcher.values()), caf_roll)
        return list(chain.from_iterable(sample))
    ...
    def survival_gear(sg_roll):
        ...
        sample = random.sample(list(sg_switcher.values()), sg_roll)
        return list(chain.from_iterable(sample))

    # Select 2 unique categories and pick 1-10 items from them
    roll = random.sample([food_and_drugs,
                          personal_electronics,
                          weapons_and_armor,
                          cyberware,
                          clothing_and_fashionware,
                          survival_gear], 2)
